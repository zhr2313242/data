...
# 编写 SQL 查询语句
query = "SELECT * FROM raw_category"
# 读取数据
...
# 编写 SQL 查询语句
query1 = "SELECT * FROM raw_category_id_similarity"
# 读取数据
...
def compute_similarity(i, j, data, conn):
    global b
    m = list(jieba.cut(data["name"][i]))
    for a in range(len(m) - 1, -1, -1):
        if m[a] in {'/', '(', ')', '（', '）'}:
            m.remove(m[a])

    n = list(jieba.cut(data["name"][j]))
    ...
    if similarity(m, n) > 0.5:
        cursor = conn.cursor()
        sql_select = "select * from raw_category_id_similarity where raw_category_id_Comparative_elements=%s and raw_category_id_source=%s"
        param_select = (int(data['id'][j]), int(data['id'][i]))
        cursor.execute(sql_select, param_select)
        result = cursor.fetchone()

        if not result:  # 如果查询结果为空，则插入数据
            sql_insert = "insert into raw_category_id_similarity values(%s,%s,%s,%s)"
